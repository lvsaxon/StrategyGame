...
public class Pathfinder2D : MonoBehaviour 
{
                ...
                foreach (RaycastHit h in hit)
                {
                    if (DisallowedTags.Contains(h.transform.tag))
                    {
                        if (h.point.z < maxZ)
                        {
                            ...
                            Map[j, i] = new Nodes(j, i, y, ID, x, 0, false); //Non walkable tile!
                            free = false;
                            maxZ = h.point.z;
                        }
                    }
                    ...
                    else
                    {
                        if (h.point.z < maxZ)
                        {
                            ...
                            Map[j, i] = new Nodes(j, i, y, ID, x, h.point.z, true); //walkable tile!
                            free = false;
                            maxZ = h.point.z;
                        }
                    }
    ...
    private Nodes FindClosestNode(Vector3 pos)
    {
        ...
        if (n.walkable)
        {
            return new Nodes(x, y, n.yCoord, n.ID, n.xCoord, n.zCoord, n.walkable);
        }
        else
        {
            ...
            for (int i = y - 1; i < y + 2; i++)
            {
                for (int j = x - 1; j < x + 2; j++)
                {
                    ...
                    if (i > -1 && i < Map.GetLength(1) && j > -1 && j < Map.GetLength(0))
                    {
                        if (Map[j, i].walkable)
                        {
                            return new Nodes(j, i, Map[j, i].yCoord, Map[j, i].ID, Map[j, i].xCoord, Map[j, i].zCoord, Map[j, i].walkable);
                        }
                    }
                }
            }
                    ...
                    if (i != y || j != x)
                    {
                        ...
                        if (Map[j, i].walkable)
                        {
                            ...
                            if (!OnClosedList(Map[j, i].ID))
                            {
                                ...
                                {
                                    Nodes addNode = new Nodes(Map[j, i].x, Map[j, i].y, Map[j, i].yCoord, Map[j, i].ID, Map[j, i].xCoord, Map[j, i].zCoord, Map[j, i].walkable, currentNode);
                                ...
                                else
                                {
                                    ...
                                    Nodes n = GetNodeFromOpenList(Map[j, i].ID);
